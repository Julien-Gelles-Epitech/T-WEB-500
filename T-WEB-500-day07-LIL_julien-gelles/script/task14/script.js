window.addEventListener('load', () => {
    ...
    let search_input = search_form.getElementsByTagName("input")[0];
    let search_select = search_form.getElementsByTagName("select")[0];
    let search_button = search_form.getElementsByTagName("input")[1];
    let reset_button = search_form.getElementsByTagName("input")[2];
    ...
    search_button.addEventListener("click", (e) => {
        ...
        if (search_input.value == "" && search_select.value != "all class") {
            for (let i = 0 ; i < database.length ; i++) {
                if (!database[i].classList.contains(search_select.value)) {
                    database[i].classList.add("hidden");
                }
            }
        } else if (search_input.value != "" && search_select.value == "all class") {
            for (let i = 0 ; i < database.length ; i++) {
                if (!database[i].innerText.includes(search_input.value)) {
                    database[i].classList.add("hidden");
                }
            }
        } else if (search_input.value != "" && search_select.value != "all class") {
            for (let i = 0 ; i < database.length ; i++) {
                if (!database[i].innerText.includes(search_input.value) || !database[i].classList.contains(search_select.value)) {
                    database[i].classList.add("hidden");
                }
            }
        }
